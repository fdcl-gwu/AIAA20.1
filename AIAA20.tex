...
\author{Madhu K. Sridhar,\footnote{Graduate Student, Mechanical and Aerospace Engineering, University of Alabama Huntsville, 301 Sparkman Dr, Huntsville AL 35899}
...
\begin{abstract}
    ...
    Consequently, it is suitable to study the flight of a butterfly that is characterized by relatively large wings flapping in a lower frequency. 
    The outcome of the proposed model is compared with the captured motion of a live Monarch butterfly.
    It is shown that the undulation of the abdomen increases the climb rate and the forward velocity, and the motion of Monarch butterfly yields a stable period orbit. 
\end{abstract}
...
However, there are also reports that contradict these findings: butterfly’s flight under periodic flapping motion is unstable because the butterfly cannot maintain its body pitch angle within a proper range ~\cite{Yokoyama2013,Jayakumar2018}.
...
Our results show that the trajectory of the velocity asymptotically converges to a periodic orbit, suggesting that the flapping motion captured by the Monarch butterfly yields an asymptotically stable periodic solution.
...
Define an inertial frame $\mathcal{F}_I=\{\mathbf{i}_x,\mathbf{i}_y,\mathbf{i}_z\}$, where the third axis points downward, and the first two axes span the horizontal plane (Fig. \ref{fig:FI_FB}). 
...
whose origin is located at its mass center of the body (Fig. \ref{fig:FI_FB}).
...
\nomenclature{$\mathcal{F}_B$}{body-fixed frame (see Fig. \ref{fig:FI_FB})}
...
Let $\mathcal{F}_R=\{\mathbf{r}_x,\mathbf{r}_y,\mathbf{r}_z\}$ be the frame fixed to the right wing (Fig. \ref{fig:FR}).
...
\nomenclature{$\mathcal{F}_R$}{frame fixed to the right wing (see Fig. ~\ref{fig:FR})}
...
Next, we introduce the stroke frame $\mathcal{F}_S=\{\mathbf{s}_x,\mathbf{s}_y,\mathbf{s}_z\}$, which is constructed by translating the origin of $\mathcal{F}_B$ to the center of the left wing root and the right wing root, and rotating it about $\mathbf{b}_y$ by $\beta\in[-\pi,\pi)$ (Fig. \ref{fig:FS}).
...
It is positive when $\mathbf{s}_x\cdot \mathbf{b}_z <0$.  (see Fig. \ref{fig:FS})}
...
The motion of the right wing relative to $\mathcal{F}_S$ is described by 1--3--2 Euler angles $(\phi_R,\psi_R,\theta_R)$ (Fig. \ref{fig:wing_Euler}). 
...
If the deviation angle is not zero, then the stroke plane can be chosen such that the distance between the wing tip and the stroke plane, after integrated over a stroke, is minimized. 
...
Similarly, let $\mathcal{F}_L$ be the frame fixed to the left wing (Fig. \ref{fig:FL}).
...
In this section, we propose a quasi-steady aerodynamic model for the butterfly flights characterized by relatively slow flapping of large wings. 
...
Let $dr$ be the infinitesimal wing segment located distanced at $r$ from the wing root (Fig. \ref{fig:wing_segment}). 
...
More specifically, consider the four cases illustrated in Fig. ~\ref{fig:lift_direction}.
...
Consider the four cases of $\alpha >0$ illustrated in Fig. \ref{fig:lift_direction}.
...
\begin{figure}[h]
    ...
    \caption{Direction of the rotational lift when $\alpha=0$ deg.}\label{fig:lift_rot}
\end{figure}

For the case of $\alpha=0$, the direction of the rotational lift is illustrated in Fig. \ref{fig:lift_rot}, where the rotational lift is always opposite to $e_3^T \dot U$.
...
\begin{align}
    \mathrm{ad}^*_\xi = \mathrm{diag}[0_{3\times 3}, -\hat\Omega, -\hat\Omega_R, -\hat\Omega_L, - \hat\Omega_A].
\end{align}
...
The resulting measurements and the fitted curve are illustrated in Fig. \ref{fig:Monarch_wing}.
...
The twelve markers are attached to a Monarch as illustrated in Fig. \ref{fig:Monarch_marker}(a),
...
For the right wing attitude, it is assumed that the wing root is located at the center of $T_1$ and $T_2$.
...
or the right wing tip is ahead of the left wing tip (Fig. \ref{fig:Monarch_WK}(a)).
...
Instead of dealing with the asymmetry in the left wing and the right wing, we multiply $\exp(7.77\hat e_3)$ to $Q_R$ and $Q_L$ such that they become symmetric in the least square sense. 
The resulting normal vector of the stroke plane lies in the $\mathbf{b}_x$--$\mathbf{b}_z$ plane, and the stroke plane angle is $\beta = 25.42 \ \si{deg}$.
...
These are illustrated in Fig. \ref{fig:Monarch_WK}(b).
The wing kinematics angles for the right wing are mostly consistent with the the left wing, except the small deviation angle. 
...
\begin{figure}
    ...
    \caption{Monarch wing kinematics. The wing kinematics angles are in degrees.}\label{fig:Monarch_WK}
\end{figure}
...
These are illustrated in Fig. \ref{fig:comp_VICON}.
...
\begin{figure}[p]
    ...
    \caption{Comparison between the QS model (blue) and the experimental data (red); shaded area corresponds to downstroke; the resultant force of the experiment is constructed by the acceleration of the thorax, multiplied by the total mass and subtracted by the gravity. The position is shown in m, velocity in m/s, forces in N, and angles in degrees. }\label{fig:comp_VICON}
\end{figure}
...
These results are illustrated at Fig. \ref{fig:comp_ab}.
While the difference is not substantial, it is shown that the abdomen undulation slightly increases the climb rate and the forward velocity.  
...
\begin{figure}[p]
    ...
    \caption{Comparison between flight with abdomen undulation (blue), without abdomen undulation (green), with abdomen undulation in the opposite phase (purple); the simulation is for 50 strokes, and the results of the first 5 strokes are illustrated for the subfigure (d) and (e). The position is shown in m, velocity in m/s, and angles in degrees.}\label{fig:comp_ab}
\end{figure}

\subsection{Asymptotic Stability}

Another interesting results of Fig. \ref{fig:comp_ab}(c) is that the trajectory of the velocity asymptotically converges to a periodic orbit, while the initial condition does not belong to the periodic orbit exactly.   
...
A quasi-steady aerodynamic model is also constructed without assuming that the flapping frequency is sufficiently large. 
...
\end{align}
 ...
 This research has been supported in parts by NSF CMMI-1761618 and NSF CMMI-1760928.
%\bibliographystyle{IEEEtran}
%\bibliography{FWUAV,/Users/tylee/Documents/BibMaster17.bib,/Users/tylee/Documents/tylee.bib,BibCK}
\bibliography{FWUAV,BibMaster17,tylee,BibCK}
